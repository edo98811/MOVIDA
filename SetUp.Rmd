...
```{r make-better-tables}
# Merge annotation data with rowData of respective SummarizedExperiment objects
colnames(annotation_df_trans) <- c("UNIPROT", "SYMBOL", "ENSEMBL")
colnames(annotation_df_prot) <- c("UNIPROT", "SYMBOL", "ENSEMBL")


# Merge annotation_df_trans with rowData of se_trans
if (!is.null(se_trans) && !is.null(annotation_df_trans)) {
  # Convert to data.frame (if not already)
  rd <- as.data.frame(rowData(se_trans))
  
  # Add rownames as a column to join on
  rd$ENSEMBL <- rownames(rd)
  
  # Perform left join
  merged <- merge(rd, annotation_df_trans, by = "ENSEMBL", all.x = TRUE)
  
  # Restore rownames after merge
  rownames(merged) <- merged$ENSEMBL
  
  # Optionally drop the join column if not needed
  merged$ENSEMBL <- NULL
  
  # Assign back to rowData
  rowData(se_trans) <- merged
}
  ...
  lapply(results_list, function(result) {
    lapply(result, function(df) {
      if (is.data.frame(df)) {
        ...
        if ("topGO_tbl" %in% names(result) && "GO.ID" %in% names(df)) {
...
```{r test movida-data, eval = F}
relationships1 <- movida_data$get_inchi_to_ensembl()
relationships2 <- movida_data$get_inchi_to_uniprot()
relationships3 <- movida_data$get_uniprot_to_ensembl()

movida_data$save_relationships("relationships")
```
