...
test_that("check_movida_list detects required dde_ objects and classes", {
...
test_that("check_movida_list_dde validates dde objects", {
...
test_that("check_contrast returns correct validation", {
  ...
  expect_false(check_contrast("A_vs_D", groups))
})

test_that("validate_contrasts returns only valid contrasts", {
  groups <- c("A", "B", "C")
  contrasts <- c("A_vs_B", "B_vs_C", "A_vs_D")
  expect_equal(validate_contrasts(contrasts, groups), c("A vs B", "B vs C"))
})
